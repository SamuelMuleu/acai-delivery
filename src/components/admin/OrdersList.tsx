            ...
            <tbody className="divide-y divide-gray-200 md:divide-y-0">
              {contextOrders.map((order: Order) => (
                <tr key={order.id} className="block md:table-row">
                  ...
                  {/* VISUALIZAÇÃO MOBILE (CARD) - JÁ FUNCIONAL */}
                    ...
                    <div className="p-4 space-y-4">
                      <div className="flex justify-between items-center">
                        ...
                        <span className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColor(order.status)}`}>
                          {order.status ? capitalizeFirstLetter(order.status) : 'N/A'}
                        </span>
                      </div>
                      <div>
                        {order.produtos?.map((produtoPedido, prodIndex) => (
                          <div key={produtoPedido.id || prodIndex} className="mb-2 last:mb-0 text-sm">
                            <div className="font-medium text-gray-800">{produtoPedido.quantity || 1}x {produtoPedido.produto.nome} {produtoPedido.tamanho ? `(${produtoPedido.tamanho})` : ''}</div>
                            {produtoPedido.complementos?.map((compEscolhido: any, compIndex: any) => (
                              <div key={compEscolhido.id || compIndex} className="pl-3 text-xs text-gray-600">
                                {compEscolhido.complemento.tipo.toLowerCase() === 'fruta' ? `Batido com: ${compEscolhido.complemento.nome}` : `${capitalizeFirstLetter(compEscolhido.complemento.tipo)}: ${capitalizeFirstLetter(compEscolhido.complemento.nome)}`}
                              </div>
                            ))}
                          </div>
                        ))}
                      ...
                      <div className="text-sm text-gray-600 space-y-2 border-t border-gray-100 pt-3">
                        ...
                        <div className="flex items-center">
                          ...
                          <span>{order.metodoPagamento || 'N/A'}</span>
                          {order.metodoPagamento?.toLowerCase() === 'dinheiro' && typeof order.changeFor === 'number' && order.changeFor > 0 && (
                            <span className="text-xs text-gray-500 ml-2">(Troco p/ R$ {order.changeFor.toFixed(2).replace('.', ',')})</span>
                          )}
                        </div>
                      </div>
                      <div className="flex justify-between items-center border-t border-gray-100 pt-3">
                        <div className="flex items-center space-x-2">
                          <select value={order.status || ''} onChange={(e) => handleStatusChange(order.id.toString(), e.target.value as Order['status'])} className="border border-gray-300 rounded px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-purple-500">
                        ...
                        <div className="text-right"><span className="font-bold text-xl text-gray-800">{typeof order.total === 'number' ? `R$ ${order.total.toFixed(2).replace('.', ',')}` : 'N/A'}</span></div>
                      </div>
                    </div>
                  </td>
                  
                  {/* VISUALIZAÇÃO DESKTOP (TABELA) - AGORA PREENCHIDA */}
                    ...
                    <div>
                      {order.produtos?.map((produtoPedido, prodIndex) => (
                        <div key={produtoPedido.id || prodIndex} className="mb-2 last:mb-0">
                          <div className="font-medium">{produtoPedido.quantity || 1}x {produtoPedido.produto.nome} {produtoPedido.tamanho ? `(${produtoPedido.tamanho})` : ''}</div>
                          <div className="pl-2 text-xs text-gray-500">
                            {produtoPedido.complementos?.map((comp: any) => comp.complemento.nome).join(', ')}
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <td className="hidden md:table-cell px-6 py-4 whitespace-nowrap align-top text-sm text-gray-500">
                    <div>
                      <span>{order.metodoPagamento || 'N/A'}</span>
                      {order.metodoPagamento?.toLowerCase() === 'dinheiro' && typeof order.changeFor === 'number' && order.changeFor > 0 && (
                        <div className="text-xs text-gray-400 mt-1">(Troco p/ R$ {order.changeFor.toFixed(2).replace('.', ',')})</div>
                      )}
                    </div>
                  </td>
                  <td className="hidden md:table-cell px-6 py-4 whitespace-nowrap align-top text-sm font-medium text-gray-900">{typeof order.total === 'number' ? `R$ ${order.total.toFixed(2).replace('.', ',')}` : 'N/A'}</td>
